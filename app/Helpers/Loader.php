 ...
 *
 * @package Mrkoopie\Wp_Plugin_Boilerplate
...
class Loader
{
	 ...
	 *
	 * @var array $actions The actions to be fired
	 **/
	protected $actions = [];
	 ...
	 *
	 * @var array $filters The filters to be applied
	 **/
	protected $filters = [];
	 ...
	 *
	 * @var array $instances Instances for a filter or action
	 **/
	private $instances;
	 ...
	 * @param  integer 	$priority Optional. Default 10. Priority of when the function should be launched.
	 * @param  integer 	$accepted_args Optional. Default is 1. The number of arguments that is accepted.
	 * @return void
	 * @author Daniel Koop <mail@danielkoop.me>
	 **/
	...
	public function run()
	{
		foreach ( $this->filters as $id => $filter ) {
			add_filter( $filter['hook'], array( $this, 'execute_filter_'.$id), $filter['priority'], $filter['accepted_args'] );
		}

		foreach ( $this->actions as $id => $action ) {
			add_action( $action['hook'], array( $this, 'execute_action_'.$id ), $action['priority'], $action['accepted_args'] );
		}
	}
	 ...
	 * @param integer $id The key id or the task
